...
/* tools drawers */
.tool-overlay{position:fixed;inset:0;background:rgba(17,14,12,.28);z-index:70;}
.tool-drawer{
  position:fixed;top:0;right:0;height:100vh;width:390px;max-width:92vw;z-index:80;
  background:var(--surface);border-left:1px solid var(--border);display:flex;flex-direction:column;
  box-shadow:-8px 0 30px rgba(0,0,0,.12);
}
.tool-drawer-hdr{padding:14px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;background:var(--surface2);}
.tool-drawer-title{font-family:'Syne',sans-serif;font-weight:700;font-size:18px;letter-spacing:-.3px;}
.tool-drawer-body{padding:14px 16px;overflow:auto;display:flex;flex-direction:column;gap:10px;}
.tool-card{border:1px solid var(--border);border-radius:8px;padding:10px 12px;background:var(--surface2);}
.tool-link{font-size:13px;font-weight:600;color:var(--ink);text-decoration:none;display:block;line-height:1.4;}
.tool-link:hover{color:var(--accent);}
.tool-meta{font-family:'DM Mono',monospace;font-size:8px;color:var(--muted);margin-top:4px;letter-spacing:.3px;}
`;
...
export default function App() {
  ...
  const { data: myListingsData = [], loading: myListingsLoading, error: myListingsError } = useProperties({
    page: 1,
    pageSize: 8,
    sortBy: "created_at",
    sortOrder: "desc",
  ...
  const loadMarketFeed = useCallback(async () => {
    ...
    setMarketError(null);
    try {
      const resp = await fetch("https://hn.algolia.com/api/v1/search?query=india%20commercial%20real%20estate%20market&tags=story&hitsPerPage=8");
      if (!resp.ok) throw new Error("Market feed request failed");
      ...
      setMarketFeed(items.filter((i) => i.title).slice(0, 8));
    } catch (e) {
      setMarketError("Unable to fetch market headlines right now.");
    } finally {
  ...
  const alertItems = useMemo(() => {
    const distressAlerts = DISTRESSED.slice(0, 2).map((d) => ({
      title: d.title,
      meta: `${d.route} · ${d.aDate}`,
    }));
    const lowConfidence = LISTINGS.find((l) => l.conf < 60);
    if (lowConfidence) {
      distressAlerts.push({
        title: `Confidence drop alert: ${lowConfidence.name}`,
        meta: `Current confidence ${lowConfidence.conf}% · action recommended`,
      });
    }
    return distressAlerts.slice(0, 3);
  }, []);

  const handleToolOpen = useCallback((label) => {
    if (label === "Market Monitor") {
      setActiveTool("market");
      if (!marketFeed.length) loadMarketFeed();
      return;
    }
    if (label === "My Listings") {
      setActiveTool("listings");
      return;
    }
    if (label === "Alerts (3)") {
  ...
  }, [loadMarketFeed, marketFeed.length, showToast]);
  ...
  return (
    <>
        ...
        <div className="sidebar">
          ...
          <div className="nav-wrap">
            ...
            {[["Market Monitor"],["My Listings"],["Alerts (3)"]].map(([lbl])=>(
              <button key={lbl} className="nav-item" onClick={()=>handleToolOpen(lbl)}>
                {lbl}
              </button>
            ))}
          </div>
      ...
      {activeTool&&<div className="tool-overlay" onClick={()=>setActiveTool(null)}/>}
      {activeTool&&(
        <aside className="tool-drawer">
          <div className="tool-drawer-hdr">
            <div className="tool-drawer-title">
              {activeTool==="market"?"Market Monitor":activeTool==="listings"?"My Listings":"Alerts"}
            </div>
            <button className="btn btn-ghost btn-xs" onClick={()=>setActiveTool(null)}>Close</button>
          </div>
          <div className="tool-drawer-body">
            {activeTool==="market"&&(
              <>
                {marketLoading&&<div className="mono" style={{fontSize:9,color:"var(--muted)"}}>Loading live market headlines...</div>}
                {marketError&&<div className="mono" style={{fontSize:9,color:"var(--red)"}}>{marketError}</div>}
                {!marketLoading&&marketFeed.map((item,idx)=>(
                  <div key={`${item.title}-${idx}`} className="tool-card">
                    <a className="tool-link" href={item.url} target="_blank" rel="noreferrer">{item.title}</a>
                    <div className="tool-meta">{item.source} · {new Date(item.date).toLocaleDateString("en-IN")}</div>
                  </div>
                ))}
                {!marketLoading&&!marketError&&marketFeed.length===0&&(
                  <div className="mono" style={{fontSize:9,color:"var(--muted)"}}>No market headlines available right now.</div>
                )}
              </>
            )}
            ...
            {activeTool==="listings"&&(
              <>
                {myListingsLoading&&<div className="mono" style={{fontSize:9,color:"var(--muted)"}}>Loading your latest listings...</div>}
                {myListingsError&&<div className="mono" style={{fontSize:9,color:"var(--red)"}}>Unable to load listings ({myListingsError}).</div>}
                {!myListingsLoading&&myListingsData.map((item,idx)=>(
                  <div key={`${item.ae_property_id||idx}`} className="tool-card" onClick={()=>showToast(`Opening ${item.title||"listing"}…`)}>
                    <div style={{fontWeight:700,fontSize:13,lineHeight:1.35}}>{item.title||"Untitled Property"}</div>
                    <div className="tool-meta">{item.micro_market||"—"}, {item.city||"—"} · {item.asset_type||"—"}</div>
                  </div>
                ))}
                {!myListingsLoading&&!myListingsError&&myListingsData.length===0&&(
                  <div className="mono" style={{fontSize:9,color:"var(--muted)"}}>No listings found yet.</div>
                )}
              </>
            )}
            ...
            {activeTool==="alerts"&&(
              <>
                {alertItems.map((item,idx)=>(
                  <div key={`${item.title}-${idx}`} className="tool-card">
                    <div style={{fontWeight:700,fontSize:13,lineHeight:1.35}}>{item.title}</div>
                    <div className="tool-meta">{item.meta}</div>
                  </div>
                ))}
              </>
            )}
